...
You will regress the value of one crytocurrency, bitcoin (BTC), on another cryptocurrency, ethereum (ETH). If we call the regression coeffiecient b, then the cointegration vector is simply (1,-b). Then perform the ADF test on BTC −b ETH. Bitcoin and Ethereum prices are pre-loaded in DataFrames BTC and ETH.
...
BTC = pd.read_csv('../datasets/BTC.csv', index_col=0, parse_dates=True)
ETH = pd.read_csv('../datasets/ETH.csv', index_col=0, parse_dates=True)
