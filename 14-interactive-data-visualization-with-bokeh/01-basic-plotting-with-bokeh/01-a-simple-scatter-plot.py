...
df = pd.read_csv('../datasets/literacy_birth_rate.csv', usecols=['female literacy','fertility']).iloc[:-20]

fertility = list(df['fertility'].astype('float').values)
female_literacy = list(df['female literacy'].astype('float').values)
